...
import complete_pic from "../../assets/icon/complete.svg";
...
let add={};
    add._method="put";
    add.complete=1;
    add.com_date=new Date();
        axios
          .post(Source.getAddress() + "/api/fabricsbill/"+row.id,add , {
            headers: {
          ...
          .then((res) => {
const updatedRow = {...row, complete: 1};
setRecords(prev => prev.map(a => a.id === row.id ? updatedRow : a));
  const messages = {
   da: {
  title: "📌 پیام از خیاطی",
  greeting: `سلام ${row.account.name} گرامی! 👋`,
  total: `💎 مجموع مبلغ: ${row.t_amount||0} افغانی`,
  paid: `✅ مبلغ پرداخت‌شده: ${row.p_amount||0} افغانی`,
  remaining: `⏳ مبلغ باقیمانده: ${row.d_amount||0} افغانی`,
  dates: `📅 تاریخ آغاز: ${row.s_date || '-'}\n📅 تاریخ سپردن لباس: ${row.e_date || '-'}`,
  ready: "🧵 لباس‌تان آماده است، لطفاً برای گرفتنش مراجعه نمایید.",
...
,
    ...
    en: {
      ...
      greeting: `Hello ${row.account.name}! 👋`,
    ...
    try {
      const response = await axios.get(Source.getAddress() + "/api/fabricsbill", {
        ...
        params: {  
          ...
          state:selectedstate?.id || 'complete',
          ...
          selector: (row) => (
            <button
              onClick={() => {
                complete(row);
                // console.log(row);
                // if(row.benefit_tailor){
                // setBenefit(row.benefit_tailor);
                // }
                // setForm(row);
...
className="bg-light d-flex flex-column flex-lg-row justify-content-lg-between align-items-start align-items-lg-center"
      ...
      >
        <h2 className="m-2 fw-bold mb-4"><FormattedMessage id="Complete Section"/>
        </h2>
       ...
       />
        ...
        <UpdateSewingOrder optionT={tailor} setOptionT={settailor} show={UpdateModel} close={()=>setUpdateModel(false)} moneys={money} setmoneys={setmoney} options1={optionsItem} setOption1={setOptionItem} form={form} setForm={setForm} addItem={addItem} setAddItem={setAddItem} shoppingCart={shoppingCart} setShoppingCart={setShoppingCart} c_ok={true}
